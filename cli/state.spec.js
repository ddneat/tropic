...
miniTest('createState returns action createIteration', () => {
  const actions = createCliState()
  assert.strictEqual(Object.prototype.toString.call(actions.createIteration), '[object Function]')
})

miniTest('createIteration returns iteration action addPass', () => {
  const actions = createCliState()
  const iterationActions = actions.createIteration()
  assert.strictEqual(Object.prototype.toString.call(iterationActions.addPass), '[object Function]')
})

miniTest('createIteration returns iteration action addFail', () => {
  const actions = createCliState()
  const iterationActions = actions.createIteration()
  assert.strictEqual(Object.prototype.toString.call(iterationActions.addFail), '[object Function]')
})

miniTest('createIteration returns iteration action addReport', () => {
  const actions = createCliState()
  const iterationActions = actions.createIteration()
  assert.strictEqual(Object.prototype.toString.call(iterationActions.addReport), '[object Function]')
})

miniTest('getState returns initial state', () => {
  const actions = createCliState()
  assert.deepStrictEqual(actions.getState(), { iterations: [] })
})

miniTest('createIteration adds a new instance to the state', () => {
  const actions = createCliState()
  actions.createIteration()
  assert.strictEqual(actions.getState().iterations.length, 1)
})
...
miniTest('createIteration adds each time a new instance to the state', () => {
  ...
  actions.createIteration()
  actions.createIteration()
  assert.strictEqual(actions.getState().iterations.length, 2)
})
...
miniTest('createIteration adds instance with property failCount', () => {
  ...
  assert.strictEqual(iteration.failCount, 0)
})
...
miniTest('createIteration adds instance with property passCount', () => {
  ...
  actions.createIteration()
  const iteration = actions.getState().iterations[0]
  assert.strictEqual(iteration.passCount, 0)
})
...
miniTest('createIteration adds instance with property files', () => {
  ...
  actions.createIteration()
  const iteration = actions.getState().iterations[0]
  assert.deepStrictEqual(Object.prototype.toString.call(iteration.files), '[object Object]')
})
...
miniTest('createIteration adds instance with initial properties', () => {
  ...
  actions.createIteration()
  const iteration = actions.getState().iterations[0]
  assert.deepStrictEqual(iteration, {
    failCount: 0,
    passCount: 0,
    files: {}
  })
...
miniTest('iteration addPass increments the passCount', () => {
  ...
  assert.strictEqual(actions.getState().iterations[0].passCount, 1)
})
...
miniTest('iteration addPass creates a file and pushes into pass', () => {
  ...
  assert.deepStrictEqual(files['test.js'], {
...
miniTest('iteration addFail increments the failCount', () => {
  ...
  assert.strictEqual(actions.getState().iterations[0].failCount, 1)
})
...
miniTest('iteration works with multiple files', () => {
  ...
  const iteration = actions.getState().iterations[0]
  assert.strictEqual(iteration.passCount, 2)
  assert.strictEqual(iteration.failCount, 1)
  assert.deepStrictEqual(iteration.files, {
    'first.js': {
